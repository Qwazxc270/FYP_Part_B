#!/usr/bin/env python3
"""
Cone fusion node: merges LiDAR, depth-camera and RGB-camera cone
detections into a single detection stream.
...
and Python 2.7 (ROS Melodic, QCar hardware). Two constraints follow:
  ...
  * All classes inherit from object (new-style).
"""
...
def mm(*mats):
    ...
    '@' operator, which is Python 3.5+ only.
        ...
        mm(A, B, C)  is equivalent to  A @ B @ C
    """
    out = mats[0]
    for m in mats[1:]:
        out = np.matmul(out, m)
