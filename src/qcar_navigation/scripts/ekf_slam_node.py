...
This file is written to run under BOTH Python 3 (ROS Noetic, VM/Gazebo)
and Python 2.7 (ROS Melodic, QCar hardware). Two constraints follow:
  * No '@' matrix-multiply operator - use mm() below instead.
  * All classes inherit from object (new-style), so @property works
...
def mm(*mats):
    """
    Chained matrix multiply. Python 2/3 compatible replacement for the
    '@' operator, which is Python 3.5+ only.
...
class EkfSlam(object):
    ...
    def __init__(self, x0):
        ...
        self.R_imu = np.diag([0.0654 ** 2, 0.03 ** 2])
    ...
    def update_imu(self, delta, L):
        ...
        I = np.eye(n)
        self.x = x
        self.P = mm(I - mm(K, C), P, (I - mm(K, C)).T) + mm(K, R, K.T)
    ...
    def update_yaw(self, yaw_meas, R_yaw):
        ...
        I = np.eye(n)
        self.x = x
        self.P = mm(I - mm(K, H), P, (I - mm(K, H)).T) + mm(K, R, K.T)
    ...
    def update_landmark(self, lm_idx, z_rb):
        ...
        K = mm(self.P, H.T, np.linalg.inv(S))
        self.x = self.x + mm(K, innov)
        self.x[2, 0] = wrap_to_pi(self.x[2, 0])

        I = np.eye(self.n)
        self.P = (mm(I - mm(K, H), self.P, (I - mm(K, H)).T)
                  + mm(K, self._cone_R(z_rb[0, 0]), K.T))
    ...
    def mahalanobis_distance(self, lm_idx, z_rb):
        ...
        try:
            ...
            return 1e9
        return float(mm(innov.T, S_inv, innov))
    ...
    def add_landmark(self, z_body):
        ...
        cross = mm(G_full, self.P)            # (2 x n_old)
        new_P[n_old:n_old + 2, :n_old] = cross
        new_P[:n_old, n_old:n_old + 2] = cross.T

        new_P[n_old:n_old + 2,
              n_old:n_old + 2] = (mm(G_full, self.P, G_full.T)
                                  + mm(Jz, R_body, Jz.T))
...
class EkfSlamNode(object):
    ...
    def _update_candidate(self, cand, z_world):
        ...
        R = np.eye(2) * 0.3 ** 2
        S = mm(H, cand["P"], H.T) + R
        K = mm(cand["P"], H.T, np.linalg.inv(S))
        innov = z_world - cand["mu"]
        cand["mu"] = cand["mu"] + mm(K, innov)
        I = np.eye(2)
        cand["P"] = (mm(I - mm(K, H), cand["P"], (I - mm(K, H)).T)
                     + mm(K, R, K.T))
        cand["hits"] += 1
    ...
    def _promote_candidate(self, cand_idx):
        ...
        lm_x, lm_y = self.slam.landmark_pos(lm_idx)
